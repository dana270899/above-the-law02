...
import styles from './CurrentWhackAMole.module.css'

let activeBaselineDragCursorLocks = 0

function startDragCursor() {
  activeBaselineDragCursorLocks += 1
  document.documentElement.dataset.dragCursor = 'true'
}

function stopDragCursor() {
  activeBaselineDragCursorLocks = Math.max(0, activeBaselineDragCursorLocks - 1)
  if (activeBaselineDragCursorLocks === 0) delete document.documentElement.dataset.dragCursor
}
...
const GRANDMAS: GrandmaSprite[] = [
  {
    name: 'Lech grandma',
    default: '/images/mini-game/Grandma01_default.svg',
  },
  {
    name: 'Democracy grandma',
    default: '/images/mini-game/Grandma02_default.svg',
  },
  {
    name: 'Peace grandma',
    default: '/images/mini-game/Grandma03_default.svg',
  },
]

const LIFE_FULL = '/images/mini-game/Life-full.svg'
const LIFE_EMPTY = '/images/mini-game/Life-empty.svg'
const HAMMER = '/images/mini-game/Hammer_animated_fast.gif'
const BACKGROUND = '/images/mini-game/game_bg.svg'
const OUCH_SOUND = '/sounds/Ouch01.mp3'

/** A test-only snapshot of the working mini-game at the start of this experiment. */
export function CurrentWhackAMole({
  onClose,
  onContinue,
  onMinimizeChange,
  ...
  const loseLife = useCallback(() => {
    ...
    setLifeFlickerId((previousId) => previousId + 1)
  ...
  const scheduleNextMole = useCallback(() => {
    ...
    gapTimerRef.current = window.setTimeout(() => {
      if (!runningRef.current) return
      const availableHoles = Array.from({ length: HOLE_COUNT }, (_, index) => index).filter(
        (index) => index !== previousHoleRef.current,
      )
      const holeIndex = availableHoles[Math.floor(Math.random() * availableHoles.length)]
      const grandmaIndex = nextGrandmaRef.current
      nextGrandmaRef.current = (nextGrandmaRef.current + 1) % GRANDMAS.length
      previousHoleRef.current = holeIndex
      setActiveHole(holeIndex)
      setActiveGrandma(grandmaIndex)
  ...
  const handleHoleClick = (index: number, event: MouseEvent<HTMLButtonElement>) => {
    if (!running) return
    if (hitPendingRef.current) return

    // This intentionally preserves the baseline's gap-strike behavior.
    if (activeHole === null) {
      ...
      if (board) {
        ...
        playHammerStrikeAt(
          ...
          (holes?.offsetTop ?? 0) + hole.offsetTop + hole.offsetHeight / 2 + HAMMER_HIT_Y_OFFSET,
        )
      }
            ...
            {Array.from({ length: HOLE_COUNT }).map((_, index) => {
              ...
              return (
                <button
                  ...
                  onClick={(event) => handleHoleClick(index, event)}
                ...
                <div className={styles.lives} aria-label={`${lives} lives left`}>
                  {Array.from({ length: STARTING_LIVES }).map((_, index) => (
                    <img
                      ...
                      src={index < lives ? LIFE_FULL : LIFE_EMPTY}
                      alt=""
                      draggable={false}
                    />
                  ))}
