...
export function FootageWindow({
  ...
  const playVideo = useCallback(() => {
    ...
    playback.catch(() => {
      // Never leave the evidence frozen. If audible playback is restricted,
      // keep the footage moving silently; the existing next-pointer retry
      // restores full-volume playback as soon as the browser permits it.
      video.muted = true
      video.volume = 0
      void video.play().catch(() => { /* browser requires another gesture */ })
    })
